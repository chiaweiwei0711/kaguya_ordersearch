...
const fetchJson = async (url: string, ms: number, expectKey?: string): Promise<any> => {
  ...
  try {
    ...
    // 邊緣層會在答案上蓋「這是回答哪個問題的」；對不上＝快取拿錯份（2026-09-17 白白白／瓦多事故）→ 當失敗，退回直打
    if (expectKey && d._edgeKey && d._edgeKey !== expectKey) throw new Error("edge key mismatch");
    return d;
...
export const gasGet = async (up: Upstream, params: Record<string, string>, opts: GasOpts = {}): Promise<any> => {
  ...
  if (!isLocal) {
    ...
    try { return await fetchJson(`${EDGE[up]}?${q2.toString()}`, ms, `${EDGE[up]}?${qs.toString()}`); }
