...
interface Props {
  ...
  loading?: boolean;
  preview?: boolean;     // 首頁預覽模式（只顯示前幾筆 + More）
  onMore?: () => void;   // 預覽的 More 進完整列表
  onBack?: () => void;   // 列表頁返回首頁
}

const GroupOrderList: React.FC<Props> = ({ teams, onSelect, loading, preview, onMore, onBack }) => {
  const sorted = [...teams].sort((a, b) => Number(isOpen(b)) - Number(isOpen(a)));
  const shown = preview ? sorted.slice(0, 3) : sorted;

  return (
    <div className="w-full max-w-lg mx-auto">
      <div className="bg-[#fff170] rounded-[40px] px-5 sm:px-7 py-8 relative">
        {!preview && onBack && (
          <button
            onClick={onBack}
            aria-label="返回"
            ...
            return (
              <button
                ...
                className={`w-full text-left rounded-2xl px-5 py-4 min-h-[64px] flex items-center justify-between gap-3 transition ${
                  open ? "bg-white shadow-sm cursor-pointer active:scale-[0.98]" : "bg-gray-200/70 cursor-default"
                }`}
              >
