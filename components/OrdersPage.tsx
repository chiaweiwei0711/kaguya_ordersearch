...
const OrdersPage: React.FC<Props> = ({ searchQuery, setSearchQuery, onSearch, searchNotice, boundNick, onGuide, onFaq, onAbout }) => {
  const row = "flex items-center gap-3 px-5 py-4 border-t-2 border-[#283d3e]/10 font-[900] text-[#283d3e] text-[15px] active:bg-[#e9f5f6] transition";
    ...
    <div className="flex flex-col items-center w-full animate-fade-in-up pt-20 pb-4">
      ...
      <div className="w-full max-w-md px-4">
        <div className="bg-white rounded-full p-2 flex items-center gap-2 border border-black transition-transform focus-within:-translate-y-1">
          ...
          <button onClick={onSearch} aria-label="查詢" className="bg-[#e868a0] text-[#283d3e] w-12 h-12 rounded-full border border-black flex items-center justify-center shrink-0 active:scale-90 transition-transform">
        ...
        {boundNick ? (
          <div className="mt-4 bg-white rounded-2xl px-4 py-3 flex items-center gap-2.5">
            <CheckCircle2 className="w-5 h-5 stroke-[3px] text-[#49d5df] shrink-0" />
            <div className="min-w-0 flex-1">
              <div className="font-[900] text-[#283d3e] text-sm truncate">已綁定：{boundNick}</div>
              <div className="font-bold text-[#283d3e]/60 text-[12px]">從官賴進來會自動認出你</div>
            </div>
            <button onClick={logoutLine} className="shrink-0 text-[#283d3e]/55 font-[900] text-xs underline underline-offset-2 active:opacity-60">
              不是我／登出
            </button>
          </div>
        ) : (
      ...
      <div className="w-full max-w-md px-4 mt-7">
        <div className="bg-white rounded-3xl overflow-hidden">
          <div className="px-5 pt-4 pb-2 font-[900] text-[13px] tracking-widest text-[#283d3e]/55">購物說明</div>
          ...
          <div className="px-5 pt-4 pb-2 font-[900] text-[13px] tracking-widest text-[#283d3e]/55 border-t-2 border-[#283d3e]/10">聯絡我們</div>
